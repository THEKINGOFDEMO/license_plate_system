...
class CRNN(nn.Module):
    ...
    def __init__(
        ...
        super().__init__()
        if img_height <= 0:
            raise ValueError("img_height must be positive.")
        ...
        with torch.no_grad():
            dummy = torch.zeros(1, num_channels, img_height, 160)
            features = self.cnn(dummy)
            _, channels, height, _ = features.shape
            self.sequence_feature_dim = channels * height
    ...
    def forward(self, images: torch.Tensor) -> torch.Tensor:
        ...
        features = features.view(width, batch_size, channels * height)
